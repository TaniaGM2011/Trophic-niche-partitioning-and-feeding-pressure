# =========================================================
# Curvas de acumulación de presas + ajuste de modelos
# Figura final en inglés, con leyenda combinada y tamaños ajustados
# =========================================================

# Limpiar entorno (opcional, recomendado para evitar objetos viejos)
rm(list = ls())
...
run_all_for_species <- function(sp_name){
  ...
  df_sp <- df %>% 
    dplyr::filter(Especie == sp_name) %>% 
    dplyr::arrange(Estomago)
  ...
  # Etiquetas de leyenda en inglés
  ...
  # Datos para leyenda y líneas
  obs_line_df <- acc_df %>% 
    dplyr::transmute(n, y = Sobs, Serie = obs_label)
  
  fit_line_df <- curve_best_df %>% 
    dplyr::transmute(n, y = S_hat, Serie = "Clench model")
  
  # Fijar niveles de la leyenda para evitar inconsistencias con patchwork
  obs_line_df$Serie <- factor(obs_line_df$Serie, levels = legend_limits)
  fit_line_df$Serie <- factor(fit_line_df$Serie, levels = legend_limits)
  
  # Posición manual de etiquetas de panel
  label_x <- 31.5
  label_y <- if (sp_name == "S. acapulcoensis") 97 else 98
  # Posición manual de etiquetas de panel
  label_x <- 31.5
  label_y <- if (sp_name == "S. flavilatus") 97 else 98
  ...
  p <- ggplot2::ggplot(acc_df, aes(n, Sobs)) +
    ...
    ggplot2::geom_point(
      size = 2, color = sp_col, show.legend = FALSE
    ) +
    ggplot2::geom_line(
      data = obs_line_df,
      aes(n, y, color = Serie, linetype = Serie),
      linewidth = 0.7,
      show.legend = TRUE
    ) +
    ggplot2::geom_line(
    ...
    ggplot2::theme(
      axis.title = element_text(size = 11, family = BASE_FAMILY, color = "black"),
      axis.text  = element_text(size = 9, family = BASE_FAMILY, color = "black"),
      legend.position = "top",
      legend.title = element_blank(),
      legend.text = element_text(size = 11, family = BASE_FAMILY, color = "black"),
...
# =========================================================
xmax_global <- max(
  purrr::map_dbl(resultados, ~ max(.x$plot$data$n, na.rm = TRUE)),
  na.rm = TRUE
)

plots_sin_titulo <- purrr::imap(
  resultados,
  ~ .x$plot +
...
# una sola leyenda combinada + tamaños finales
fig_2col <- fig_2col & 
  ggplot2::theme(
    legend.position = "top",
    legend.text = element_text(size = 10, family = BASE_FAMILY, color = "black"),
    legend.key.size = grid::unit(0.9, "cm"),
...
ggplot2::ggsave(
  filename = "F:/Doctorado/TESIS version 3/Capitulo 2.- corregido/figuras/modelo_clench2.tiff",
  plot     = fig_2col,
