...
# ============================
# EXCLUIR Microplasticos
# ============================
cols_items <- setdiff(
  names(df),
  c("Estomago", "Especie", "Microplasticos")   # <-- aquí se excluye
)

# OPCIÓN ROBUSTA (si el nombre varía: microplásticos, MICRO...)
# cols_items <- setdiff(
#   names(df),
#   c("Estomago", "Especie", grep("micro", names(df), ignore.case = TRUE, value = TRUE))
# )
...
# Distancia / similitud Bray–Curtis
...
# ============================
# B) BRAY–CURTIS INDIVIDUO-A-INDIVIDUO
# ============================
A <- df %>% filter(Especie == "S. acapulcoensis") %>%
  select(all_of(cols_items)) %>% as.matrix()

B <- df %>% filter(Especie == "S. flavilatus") %>%
  select(all_of(cols_items)) %>% as.matrix()

# Función de similitud BC
bray_sim_vec <- function(a, b) 1 - sum(abs(a - b)) / sum(a + b)

# Todas las comparaciones A vs B
